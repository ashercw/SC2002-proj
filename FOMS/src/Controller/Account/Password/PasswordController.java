...
public class PasswordController extends JPanel{
	 ...
	 * information like
	 * the EmployeeType, userID, and password corresponds to data stored in the
	 * EmployeeRepo.
	 * 
	 ...
	 * @return int with a value of -1, 0, or 1 (-1: User does not exist, 0: password
	 *         entered is wrong,
	...
	public static int checkCredentials(EmployeeType empType, String inputPassw, String inputUserID) {
		...
		try {
			...
			for (int i = 0; i < employeeList.size(); i++) {
				...
				if (isUserValid) {
					if (CredentialsValidator.checkPassW(emp, inputPassw))
