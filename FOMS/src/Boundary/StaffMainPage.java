 ...
 * with orders.
 * This class handles operations like displaying new orders, processing orders,
...
public class StaffMainPage {
    ...
    /**
     * Constructs a StaffMainPage object for a given staff member identified by
     * staffID.
     * It initializes the list of orders and loads staff details from the
     * "EmployeeRepo.txt" file.
     *
    ...
    /**
     * Displays the main page interface for the staff, allowing them to choose
     * between
    ...
    public static void displayNewOrders() {
         ...
         * order.getOrderLine().forEach((item) ->
         * System.out.println(item.getItem().getFoodItemName() + ", Quantity: " +
    ...
    /**
     * Processes an order by updating its status to "Ready to Pickup" based on the
     * given order ID.
     *
    ...
    public static void processOrder(int orderId) {
        ...
        /*
         * for (Order order : orders) {
         * if (order.getOrderID() == orderId && (order.getOrderStatus() ==
         * OrderStatus.ORDERPLACED || order.getOrderStatus() == OrderStatus.PROCESSING))
         * {
    ...
    public static void trackOrderStatus(int orderID) {
        ...
        for (int i = 0; i < fullOrderL.size(); i++) {
            ...
            {    //System.out.println("Order ID " + orderID + " is now ready to pick up.");
                switch (currOrder2.getOrderStatus()) {
